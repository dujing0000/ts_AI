...
text = input("質問または指示を入力してください:")

# f-stringを使用して変数を埋め込みます
SYSTEM_PROMPT = f"""
...
def generate_summary(text_content, image_list_text):
    ...
    print(f"🚀 AI生成開始 (長文処理中)...")
    response = llm.invoke(prompt_text)
    return response.content

# ==========================================
# PDF保存 (改良版：表の折り返しとレイアウト調整)
# ==========================================
def parse_markdown_table(markdown_lines):
    """Markdownの表テキストを解析してリストのリストにする"""
    data = []
...
def format_inline_bold(text):
    """
    Markdownの太字をReportLabタグに変換し、
    ReportLabがエラーを起こす <br> を <br/> に修正する
    """
    if not text: return ""
    
    # 1. <br> タグの正規化
    # ReportLabは <br> (閉じなし) でクラッシュするため、<br/> に置換します
    text = re.sub(r'<br\s*/?>', '<br/>', text, flags=re.IGNORECASE)
    
    # 2. Markdownの太字(**text**)をタグ(<b>text</b>)に変換
...
def create_paragraph_table(raw_data_lines, styles, available_width):
    """
    Markdown行リストを受け取り、Paragraph化して自動改行テーブルを作成。
    行数が少ない場合は KeepTogether でラップして分割を防ぐ。
    """
    ...
    for row in parsed_data:
        row_str = "".join(row)
        if not row_str or not all(c in '-: ' for c in row_str):
    ...
    for i, row in enumerate(normalized_data):
        ...
        for cell_text in row:
            # ヘッダー(0行目)は中央寄せ、他は左寄せ
            s = style_cell_center if i == 0 else style_cell_left
            formatted_text = format_inline_bold(cell_text)
            p = Paragraph(formatted_text, s)
    ...
    # 【追加修正】
    # 行数が30行以下なら、途中で切れないように KeepTogether でまとめる
    # これにより「ページに入り切らない場合は次のページに表全体を送る」挙動になる
...
def save_to_pdf(markdown_text, image_data_dict, original_filename):
    ...
    # --- スタイル定義 (keepWithNextを追加) ---
    # keepWithNext=True にすると、その段落がページの最後に来た時、次の要素を連れて次のページへ移動します。
    # これにより「見出し」と「その下の図/表」が分離するのを防ぎます。
    ...
    for line in lines:
        ...
        if line.startswith('|'):
            ...
            if in_table:
                ...
                if t:
                    ...
                    story.append(t) # ここでKeepTogether化された表が追加される
        ...
        if img_match:
            ...
            if label_key in image_data_dict:
                ...
                try:
                    ...
                    # 画像とキャプションをセットにして、ページをまたがないようにする
        ...
        if line.startswith('# '):
            ...
            text = line.replace('#### ', '').strip()
            # H4などは本文扱いだが、強調したいので太字
            story.append(Paragraph(f"<b>{text}</b>", style_body))

        # --- 箇条書き ---
        elif line.startswith('- ') or line.startswith('* '):
            text = format_inline_bold(line[2:])
            story.append(Paragraph(f"• {text}", style_bullet))
            
        # --- 通常本文 ---
        else:
            text = format_inline_bold(line)
    ...
    try:
        doc.build(story)
        print(f"💾 PDF保存完了 (Formatted): {output_path}")
        return True
    except Exception as e:
        print(f"❌ PDF保存エラー: {e}")
        import traceback
